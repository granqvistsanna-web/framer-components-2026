/**
 * Shared WebGL shader utility for Framer components.
 * Provides a React hook that creates a canvas with a WebGL2 shader program,
 * handles resize, animation loop, and cleanup.
 *
 * All utilities inlined from @paper-design/shaders so Framer can resolve imports.
 */
...
// Color parsing (getShaderColorFromString)
...
export function getShaderColorFromString(colorString: string | number[]): [number, number, number, number] {
...
// ─── GLSL Snippets (from @paper-design/shaders shader-utils) ─
export const glsl_declarePI = `
#define TWO_PI 6.28318530718
#define PI 3.14159265358979323846
`

export const glsl_rotation2 = `
vec2 rotate(vec2 uv, float th) {
  return mat2(cos(th), sin(th), -sin(th), cos(th)) * uv;
}
`

export const glsl_proceduralHash11 = `
...
export const glsl_proceduralHash21 = `
  float hash21(vec2 p) {
...
export const glsl_proceduralHash22 = `
  vec2 hash22(vec2 p) {
    p = fract(p * vec2(0.3183099, 0.3678794)) + 0.1;
    p += dot(p, p.yx + 19.19);
...
export const glsl_textureRandomizerR = `
...
export const glsl_textureRandomizerGB = `
...
export const glsl_colorBandingFix = `
  color += 1. / 256. * (fract(sin(dot(.014 * gl_FragCoord.xy, vec2(12.9898, 78.233))) * 43758.5453123) - .5);
`

export const glsl_simplexNoise = `
...
// ─── Vertex Shader ──────────────────────────────────────────
// Extracted from @paper-design/shaders ShaderMount
...
function compileShader(gl: WebGL2RenderingContext, type: number, source: string): WebGLShader | null {
    // Upgrade precision if device needs it
...
function setUniforms(
    ...
    for (const [key, value] of Object.entries(uniforms)) {
        ...
        if (value instanceof HTMLImageElement) {
            ...
            if (arLoc && value.naturalWidth > 0) {
                gl.uniform1f(arLoc, value.naturalWidth / value.naturalHeight)
            }
...
export function useShaderCanvas(
    ...
    // Store options in a ref for the animation loop
    ...
    const render = useCallback((time: number) => {
        ...
        s.lastTime = time
        if (!opt.paused && opt.speed !== 0) {
            s.currentFrame += dt * opt.speed
        }
    ...
    useEffect(() => {
        ...
        const onVis = () => {
            ...
            } else if (s.rafId === null && !optRef.current.paused) {
                s.lastTime = performance.now()
                s.rafId = requestAnimationFrame(render)
            }
        }
    ...
    // Update speed/paused — restart or stop RAF
    useEffect(() => {
        const s = stateRef.current
        if (s.disposed || !s.gl) return
        if (!options.paused && options.speed !== 0 && s.rafId === null) {
            s.lastTime = performance.now()
            s.rafId = requestAnimationFrame(render)
        }
        if ((options.paused || options.speed === 0) && s.rafId !== null) {
