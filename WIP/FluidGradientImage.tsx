...
void main() {
    ...
    // mask: 1 inside image area, 0 in overflow padding
    // transition lives entirely in the overflow zone so the image edge stays sharp
    float inImage = smoothstep(-0.005, 0.0, imageUv.x) * smoothstep(-0.005, 0.0, 1.0 - imageUv.x)
                  * smoothstep(-0.005, 0.0, imageUv.y) * smoothstep(-0.005, 0.0, 1.0 - imageUv.y);
...
export default function FluidGradientImage(props: FluidGradientImageProps) {
    const {
        image,
        effectColor1 = "#0D9488",
        effectColor2 = "#A78BFA",
    ...
    React.useEffect(() => {
        ...
        function render(now: number) {
            ...
            let allDecayed = pointerActiveFade < 0.001
            if (allDecayed) {
                for (let i = 0; i < TRAIL_LENGTH; i++) {
    ...
    return (
        ...
        >
            {loadError ? (
                ...
                >
                    {`Fluid Gradient Image: ${loadError}`}
                </div>
            ) : null}
        </div>
    )
...
FluidGradientImage.displayName = "Fluid Gradient Image"

addPropertyControls(FluidGradientImage, {
    image: {
