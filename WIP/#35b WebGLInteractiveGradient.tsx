...
void main() {
    ...
    // Domain warping: use noise to organically distort UV before color math
    float warpStrength = uNoiseAmount * 0.8;
    vec2 warpedUv = uv;
    float q1 = fbm(uv * 1.5 + t * 0.12);
    float q2 = fbm(uv * 1.5 + vec2(5.2, 1.3) + t * 0.1);
    warpedUv += warpStrength * vec2(q1 - 0.5, q2 - 0.5);
    // Second warp pass for extra organic flow
    float r1 = fbm(warpedUv * 1.8 + vec2(1.7, 9.2) + t * 0.08);
    float r2 = fbm(warpedUv * 1.8 + vec2(8.3, 2.8) + t * 0.06);
    warpedUv += warpStrength * 0.5 * vec2(r1 - 0.5, r2 - 0.5);

    float d = -t * 0.5;
    float a = 0.0;
    for (float i = 0.0; i < 8.0; ++i) {
        a += cos(i - d - a * warpedUv.x);
        d += sin(warpedUv.y * i + a);
    }
    d += t * 0.5;

    float mixer1 = cos(warpedUv.x * d) * 0.5 + 0.5;
    float mixer2 = cos(warpedUv.y * a) * 0.5 + 0.5;
    float mixer3 = sin(d + a) * 0.5 + 0.5;

    // Softness blending
    float smoothAmount = clamp(uSoftness * 0.15, 0.0, 0.8);
    mixer1 = mix(mixer1, 0.5, smoothAmount);
    mixer2 = mix(mixer2, 0.5, smoothAmount);
    mixer3 = mix(mixer3, 0.5, smoothAmount);

    // Smoothstep for soft transitions
    mixer1 = mixer1 * mixer1 * (3.0 - 2.0 * mixer1);
    mixer2 = mixer2 * mixer2 * (3.0 - 2.0 * mixer2);
    mixer3 = mixer3 * mixer3 * (3.0 - 2.0 * mixer3);

    vec3 col = mix(uColor1, uColor2, mixer1);
    col = mix(col, uColor3, mixer2);
    col = mix(col, uColor4, mixer3 * 0.6);
...
function resolveConfig(props: WebGLInteractiveGradientProps): RuntimeConfig {
    ...
    return {
        ...
        speed: props.styleSettings?.speed ?? props.speed ?? 0.4,
        brushSize: props.advanced?.brushSize ?? props.brushSize ?? 40,
        brushStrength: props.advanced?.brushStrength ?? props.brushStrength ?? 0.3,
        distortionAmount:
            props.styleSettings?.distortionAmount ?? props.distortionAmount ?? 0.8,
        ...
        noiseAmount: props.styleSettings?.noiseAmount ?? props.noiseAmount ?? 0.3,
        colorIntensity: props.styleSettings?.colorIntensity ?? props.colorIntensity ?? 1,
        softness: props.styleSettings?.softness ?? props.softness ?? 0.5,
...
export default function WebGLInteractiveGradient(props: WebGLInteractiveGradientProps) {
    ...
    if (
        ...
        prevConfig.noiseAmount !== resolvedConfig.noiseAmount ||
    ...
    useEffect(() => {
        ...
        const displayMaterial = new THREE.ShaderMaterial({
            uniforms: {
                ...
                uNoiseAmount: { value: configRef.current.noiseAmount },
            },
            vertexShader,
...
WebGLInteractiveGradient.displayName = "#35b WebGL Soft Gradient"
